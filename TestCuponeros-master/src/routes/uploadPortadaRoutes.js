...
router.post('/portadas/:id', upload.single('imagen'), async (req, res) => {
  try {
    ...
    const query = 'INSERT INTO Portada (nombre, imagen, descripcion, user_id) VALUES (?, ?, ?, ?)';
    const [result] = await connection.execute(query, [nombre, imagen, descripcion, user_id]);

    connection.release();

    res.status(200).json({ user_id, nombre, descripcion, imagen });
  } catch (error) {
    console.error(error);
...
router.put('/portadas/:id', upload.single('imagen'), async (req, res) => {
  try {
    ...
    const query = 'UPDATE Portada SET nombre = ?, imagen = ?, descripcion = ?, user_id = ? WHERE id = ?';
    const [result] = await connection.execute(query, [nombre, descripcion, user_id,imagen]);
...
router.get('/portadas/:id', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.query('SELECT imagen FROM Portada WHERE user_id = ?', [user_id]);
...
router.delete('/portadas/:id', async (req, res) => {
  try {
    ...
    const query = 'DELETE FROM Portada WHERE user_id = ?';
