...
router.post('/logos/:id', upload.single('imagen'), async (req, res) => {
  try {
    ...
    const query = 'INSERT INTO Logo (nombre, imagen, descripcion, user_id) VALUES (?, ?, ?, ?)';
...
router.put('/logos/:id', upload.single('imagen'), async (req, res) => {
  try {
    ...
    const query = 'UPDATE Logo SET nombre = ?, imagen = ?, descripcion = ?, user_id = ? WHERE id = ?';
...
router.get('/logos/:id', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.query('SELECT imagen FROM Logo WHERE user_id = ?', [user_id]);
...
router.delete('/logos/:id', async (req, res) => {
  try {
    ...
    const query = 'DELETE FROM Logo WHERE user_id = ?';
